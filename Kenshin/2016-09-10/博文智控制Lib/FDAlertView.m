...
//        [_screenBtn addTarget:self action:@selector(closeSelfView) forControlEvents:UIControlEventTouchUpInside];
...
- (void)closeSelfView
{
    [self hide];//其实这里可能不应该用隐藏，而是直接销毁自身
}
...
- (instancetype)initWithTitle:(NSString *)title icon:(UIImage *)icon message:(NSString *)message delegate:(id<FDAlertViewDelegate>)delegate buttonTitles:(NSString *)buttonTitles, ... {
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        ...
        //一下代码 用于点击空白处 关闭弹出框[由于view的层级关系，所以下面的btn应该添加在弹出框之前]
        _screenBtn = [[UIButton alloc]initWithFrame:self.frame];
        [_screenBtn addTarget:self action:@selector(closeSelfView) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_screenBtn];
...
- (void)show
{
    ...
    //这里的两行代码 替换下面注释掉的一行代码，如果用西面的代码 将会出现一个情况：通过ActionSheet打开的本实例 将显示一下然后立即消失。
    //具体原因应该是UIWindow的机制
