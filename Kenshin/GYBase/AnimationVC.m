...
- (void)initAnimationVCUI
{
    ...
    [Tools setCornerRadiusWithView:self.sakuraiHeader andAngle:sizeHeader/2];
    [self.view addSubview:self.sakuraiHeader];
    
    //1位移
    BlockButton *moveLeftRight = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10, 64 + margin_10, size_normal, height_normal)];
    [moveLeftRight setTitle:@"位移" forState:UIControlStateNormal];
    moveLeftRight.backgroundColor = [UIColor blackColor];
    
    //2渐变
    BlockButton *change = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *2 + size_normal, 64 + margin_10, size_normal, height_normal)];
    [change setTitle:@"渐变" forState:UIControlStateNormal];
    change.backgroundColor = [UIColor blackColor];
    
    //3旋转
    BlockButton *round = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *3 + size_normal *2, 64 + margin_10, size_normal, height_normal)];
    [round setTitle:@"旋转" forState:UIControlStateNormal];
    round.backgroundColor = [UIColor blackColor];
    
    //4(位移＋渐变)
    BlockButton *moveChange = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *4 + size_normal *3, 64 + margin_10, size_normal, height_normal)];
    [moveChange setTitle:@"位渐" forState:UIControlStateNormal];
    moveChange.backgroundColor = [UIColor blackColor];
    
    //5(位移＋旋转)
    BlockButton *moveRound = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *5 + size_normal *4, 64 + margin_10, size_normal, height_normal)];
    [moveRound setTitle:@"位旋" forState:UIControlStateNormal];
    moveRound.backgroundColor = [UIColor blackColor];
    
    //6（渐变＋旋转）
    BlockButton *changeRound = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *2 + size_normal, 64 + margin_10*2 + height_normal, size_normal, height_normal)];
    [changeRound setTitle:@"渐旋" forState:UIControlStateNormal];
    changeRound.backgroundColor = [UIColor blackColor];

    //7(位移＋渐变＋旋转)
    BlockButton *moveChangeRound = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *3 + size_normal *2, 64 + margin_10*2 + height_normal, 60, height_normal)];
    [moveChangeRound setTitle:@"位渐旋" forState:UIControlStateNormal];
    moveChangeRound.backgroundColor = [UIColor blackColor];

    //8 缩放
    BlockButton *smallBig = [[BlockButton alloc] initWithFrame:CGRectMake(moveRight+margin_10 *6 + size_normal *3, 64 + margin_10*2 + height_normal, 60, height_normal)];
    [smallBig setTitle:@"缩放" forState:UIControlStateNormal];
    smallBig.backgroundColor = [UIColor blackColor];
    
    //波纹动画
    wavaBtn = [[BlockButton alloc] initWithFrame:CGRectMake(0, 200, size_normal, height_normal)];
    ...
    WS(ws);
    //1位移
    moveLeftRight.block = ^(BlockButton *btn){
        [ws moveFunction];
    };
    
    //2渐变
    change.block = ^(BlockButton *btn){
        [ws change];
    };
    
    //3旋转
    round.block = ^(BlockButton *btn){
        [ws round];
    };
    
    //4位移 渐变
    moveChange.block = ^(BlockButton *btn){
        [ws moveFunction];
        [ws change];
    };
    
    //5位移 旋转
    moveRound.block = ^(BlockButton *btn){
        [ws moveFunction];
        [ws round];
    };
    
    //6渐变 旋转
    changeRound.block = ^(BlockButton *btn){
        [ws change];
        [ws round];
    };
    
    //7位移 渐变 旋转
    moveChangeRound.block = ^(BlockButton *btn){
        [ws moveFunction];
        [ws change];
        [ws round];
    };
    ...
    //缩放
    smallBig.block = ^(BlockButton *btn){
        [ws smallBig:ws.sakuraiHeader];
    };
    
    wavaBtn.block = ^(BlockButton *btn){
...
- (void)startAnimationOfWave
{
    ...
    wavaBtn.enabled = NO;
    [wavaBtn setBackgroundImage:[UIImage imageNamed:@"ququbtn"] forState:UIControlStateDisabled];
