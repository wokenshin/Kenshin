...
+(NSString *)saveImage:(UIImage *)image imageName:(NSString *)imageName andCompress:(double)compress
{
    BOOL success;
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error;
...
//    saveImage = [self thumbnailWithImageWithoutScale:image size:size];
//    saveImage = [self thumbnailWithImage:image size:CGSizeMake(image.size.width*compress, image.size.height*compress)];
    if (compress > 1 || compress < 0) {
        compress = 1;
    }
    [UIImageJPEGRepresentation(saveImage, compress) writeToFile:imageFilePath atomically:YES];//写入文件
