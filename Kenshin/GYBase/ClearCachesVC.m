...
@property (nonatomic, strong)UIButtonK          *btnClear;//清空缓存
...
- (void)initClearCachesUI
{
    ...
    _labCaches = [[UILabel alloc] initWithFrame:CGRectMake(screenWidth/2 - labWidth/2, 64 + margin_10, labWidth, 44)];
    _labCaches.textAlignment = NSTextAlignmentCenter;
    _labCaches.backgroundColor = RGB2Color(79, 190, 91);
    
    WS(ws);//防止循环引用
    _btnClear = [[UIButtonK alloc] initWithFrame:CGRectMake(screenWidth/2 - btnWidth/2, 64 + btnWidth, btnWidth, 44)];
    [_btnClear setStyleGray];
    [_btnClear setTitle:@"清除缓存" forState:UIControlStateNormal];
    _btnClear.clickButtonBlock = ^(UIButtonK *b){
        [ws clearCaches];
